...
.then (dom => {
  ...
  // Suggest keywords as text is enter in the search box
  sInput.onkeyup = () => {
    if (sInput.value === "") {
      ...
      .then(keywords => {
        ...
        if (keywords.matches.length === 0) {
          ...
          for (let word of keywords.matches) {
            ...
            suggestion.onclick = () => {
              sInput.value = word;
              suggestBox.setHTMLUnsafe.display = "none";
              sButton.click();
            }
          }
...
function getGIF(topic) {
  ...
  const key = "Xo8aRdwg72sVVNJwyePG5S0Mx7KVNQW4";
